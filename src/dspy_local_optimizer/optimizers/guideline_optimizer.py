...
class GuidelineProgram(dspy.Module):
    ...
    def __init__(self, api_key: str, model_name: str = "openai/gpt-3.5-turbo", use_optimizer: bool = True, metrics: Optional[ModelMetrics] = None):
        ...
        if use_optimizer:
            # Create debug metric for optimizer
            def debug_metric(pred: Dict[str, str], gold: Dict[str, str]) -> float:
                """Debug metric that logs inputs and returns quality score."""
                logger.debug(f"Comparing pred={pred} with gold={gold}")
                # Extract response from pred/gold, handling both direct and nested dicts
                pred_response = pred.get("response", "") if isinstance(pred, dict) else getattr(pred, "response", "")
                gold_response = gold.get("response", "") if isinstance(gold, dict) else getattr(gold, "response", "")
                result = self._calculate_response_quality(pred_response, gold_response)
